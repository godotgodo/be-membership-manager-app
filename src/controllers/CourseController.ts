...
class CourseController {
  ...
  public async addCourseToSchool(
    ...
    try {
      ...
      const savedCourse = await this.courseService.addCourseToSchool(
        schoolId,
        newCourse,
      );
  ...
  public async addMembersToCourse(
    ...
    try {
      ...
      const addedMembers = await this.courseService.addMembersToCourse(
        courseId,
        members,
      );
  ...
  public async searchMembersByName(
    ...
    try {
      const courseId = new mongoose.Types.ObjectId(req.params.id);
      const { name } = req.body;
      const members = await this.courseService.searchMembersByName(
        courseId,
        name,
      );
